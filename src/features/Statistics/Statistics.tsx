...
export default function Statistics() {
  ...
  const [timeOpts, setTimeOpts] = useState<ChartOptions>(chartOptions("time"));
  ...
  useEffect(() => {
    ...
    setTimeOpts((prevState) => {
      ...
      series[1].data = [state.getDescFeatTypeTime, state.postDescFeatTypeTime];
      series[2].data = [state.getGetPropValTime, state.postGetPropValTime];
      series[3].data = [
  ...
  useEffect(() => {
    ...
    setRequestsOpts((prevState) => {
      ...
      series[2].data = [state.getGetPropValNumber, state.postGetPropValNumber];
      series[3].data = [
  ...
  return (
    <Col md={{ size: 8, offset: 2 }} className="mt-4">
      ...
      <Panel
        content={
          state.getCapResp && (
            <HighchartsReact highcharts={Highcharts} options={timeOpts} />
          )
        }
      ...
      <Panel
        content={
          state.getCapResp && (
            <HighchartsReact highcharts={Highcharts} options={requestsOpts} />
          )
        }
